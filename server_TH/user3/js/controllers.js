...
myControllers.controller("ListCtrl", function($scope, $firebase) {
	...
	var userRef = ref.child("TH").child("user3");
	var container = $('#message_container');
	var setRef = ref.child("TH").child("user3").child("state").set("online"); // set user3 online
	var userChatNo = ref.child("TH").child("user3").child("chatNo");  //set chat count
	var userOnline = ref.child("TH").child("user3").child("onlineNo"); //set online chat count
	...
	window.onbeforeunload=function(event){
		ref.child("TH").child("user3").child("state").set("offline");  // user2 offline
    }
});