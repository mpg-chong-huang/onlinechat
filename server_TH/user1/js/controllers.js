...
myControllers.controller("ListCtrl", function($scope, $firebase) {
	...
	var userRef = ref.child("TH").child("user1");
	var container = $('#message_container');
	var setRef = ref.child("TH").child("user1").child("state").set("online"); // set user1 online
	var userChatNo = ref.child("TH").child("user1").child("chatNo");  //set chat count
	//userChatNo.set(0);
	var userOnline = ref.child("TH").child("user1").child("onlineNo"); //set online chat count
	//userOnline.set(0);
	...
	window.onbeforeunload=function(event){
		ref.child("TH").child("user1").child("state").set("offline");  // user1 offline
    }
});