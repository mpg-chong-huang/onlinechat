...
myControllers.controller("ListCtrl", function($scope, $firebase) {
	...
	var userRef = ref.child("TH").child("user2");
	var container = $('#message_container');
	var setRef = ref.child("TH").child("user2").child("state").set("online"); // set user2 online
	var userChatNo = ref.child("TH").child("user2").child("chatNo");  //set chat count
	var userOnline = ref.child("TH").child("user2").child("onlineNo"); //set online chat count
	var a = 0, b = 0,c = 0,d = 0 , check_point = 0,list_long = 0;
	...
	window.onbeforeunload=function(event){
		ref.child("TH").child("user2").child("state").set("offline");  // user2 offline
    }
});